...
var require_middleware = __commonJS({
  ".next/server/middleware.js"() {
    ...
    (self.webpackChunk_N_E = self.webpackChunk_N_E || []).push([[751], { 35: (a, b) => {
      ...
      "use strict";
      let d;
      c.r(b), c.d(b, { default: () => bU });
      var e = {};
      c.r(e), c.d(e, { default: () => bC, getRandomValues: () => bB, randomUUID: () => bA, subtle: () => bz });
      var f = {};
      async function g() {
        return "_ENTRIES" in globalThis && _ENTRIES.middleware_instrumentation && await _ENTRIES.middleware_instrumentation;
      }
      c.r(f), c.d(f, { config: () => bQ, middleware: () => bP });
      let h = null;
      async function i() {
...
Learn More: https://nextjs.org/docs/messages/node-module-in-edge-runtime`;
  ...
  `);
      ...
      var be = c(614), bf = {}, bg = Array.from({ length: 64 });
      for (let a10 = 0; a10 + 65 <= 90; a10++) {
        let b2 = String.fromCharCode(a10 + 65);
        bf[b2] = a10, bg[a10] = b2;
      }
      for (let a10 = 0; a10 + 97 <= 122; a10++) {
        let b2 = String.fromCharCode(a10 + 97), c2 = a10 + 26;
        bf[b2] = c2, bg[c2] = b2;
      }
      for (let a10 = 0; a10 < 10; a10++) {
        bf[a10.toString(10)] = a10 + 52;
        let b2 = a10.toString(10), c2 = a10 + 52;
        bf[b2] = c2, bg[c2] = b2;
      }
      bf["-"] = 62, bg[62] = "-", bf._ = 63, bg[63] = "_";
      var bh = (a10) => new TextEncoder().encode(a10), bi = (a10) => {
        let b2 = a10 + "=".repeat((4 - a10.length % 4) % 4), c2 = b2.length / 4 * 3;
        b2.endsWith("==") ? c2 -= 2 : b2.endsWith("=") && c2--;
        let d2 = new ArrayBuffer(c2), e2 = new DataView(d2);
        for (let a11 = 0; a11 < b2.length; a11 += 4) {
          let c3 = 0, d3 = 0;
          for (let e3 = a11, f3 = a11 + 3; e3 <= f3; e3++) if ("=" === b2[e3]) c3 >>= 6;
          else {
            if (!(b2[e3] in bf)) throw TypeError(`Invalid character ${b2[e3]} in base64 string.`);
            c3 |= bf[b2[e3]] << (f3 - e3) * 6, d3 += 6;
          }
          let f2 = a11 / 4 * 3;
          c3 >>= d3 % 8;
          let g2 = Math.floor(d3 / 8);
          for (let a12 = 0; a12 < g2; a12++) {
            let b3 = (g2 - a12 - 1) * 8;
            e2.setUint8(f2 + a12, (c3 & 255 << b3) >> b3);
          }
        }
        return new Uint8Array(d2);
      }, bj = (a10) => {
        let b2 = "string" == typeof a10 ? bh(a10) : a10, c2 = "";
        for (let a11 = 0; a11 < b2.length; a11 += 3) {
          let d2 = 0, e2 = 0;
          for (let c3 = a11, f3 = Math.min(a11 + 3, b2.length); c3 < f3; c3++) d2 |= b2[c3] << (f3 - c3 - 1) * 8, e2 += 8;
          let f2 = Math.ceil(e2 / 6);
          d2 <<= 6 * f2 - e2;
          for (let a12 = 1; a12 <= f2; a12++) {
            let b3 = (f2 - a12) * 6;
            c2 += bg[(d2 & 63 << b3) >> b3];
          }
        }
        return c2;
      }, bk = { encryption: { saltBits: 256, algorithm: "aes-256-cbc", iterations: 1, minPasswordlength: 32 }, integrity: { saltBits: 256, algorithm: "sha256", iterations: 1, minPasswordlength: 32 }, ttl: 0, timestampSkewSec: 60, localtimeOffsetMsec: 0 }, bl = (a10) => ({ ...a10, encryption: { ...a10.encryption }, integrity: { ...a10.integrity } }), bm = { "aes-128-ctr": { keyBits: 128, ivBits: 128, name: "AES-CTR" }, "aes-256-cbc": { keyBits: 256, ivBits: 128, name: "AES-CBC" }, sha256: { keyBits: 256, name: "SHA-256" } }, bn = "Fe26.2", bo = (a10, b2) => {
        if (b2 < 1) throw Error("Invalid random bits count");
        return ((a11, b3) => {
          let c2 = new Uint8Array(b3);
          return a11.getRandomValues(c2), c2;
        })(a10, Math.ceil(b2 / 8));
      }, bp = async (a10, b2, c2, d2, e2, f2) => {
        let g2 = bh(b2), h2 = await a10.subtle.importKey("raw", g2, { name: "PBKDF2" }, false, ["deriveBits"]), i2 = bh(c2);
        return await a10.subtle.deriveBits({ name: "PBKDF2", hash: f2, salt: i2, iterations: d2 }, h2, 8 * e2);
      }, bq = async (a10, b2, c2) => {
        var d2;
        if (!(null == b2 ? void 0 : b2.length)) throw Error("Empty password");
        if (null == c2 || "object" != typeof c2) throw Error("Bad options");
        if (!(c2.algorithm in bm)) throw Error(`Unknown algorithm: ${c2.algorithm}`);
        let e2 = bm[c2.algorithm], f2 = {}, g2 = null != (d2 = c2.hmac) && d2, h2 = g2 ? { name: "HMAC", hash: e2.name } : { name: e2.name }, i2 = g2 ? ["sign", "verify"] : ["encrypt", "decrypt"];
        if ("string" == typeof b2) {
          ...
          if (!d3) {
            ...
            d3 = [...new Uint8Array(bo(a10, b3))].map((a11) => a11.toString(16).padStart(2, "0")).join("");
          }
          let g3 = await bp(a10, b2, d3, c2.iterations, e2.keyBits / 8, "SHA-1");
          f2.key = await a10.subtle.importKey("raw", g3, h2, false, i2), f2.salt = d3;
        } else {
          if (b2.length < e2.keyBits / 8) throw Error("Key buffer (password) too small");
          f2.key = await a10.subtle.importKey("raw", b2, h2, false, i2), f2.salt = "";
        }
        return c2.iv ? f2.iv = c2.iv : "ivBits" in e2 && (f2.iv = bo(a10, e2.ivBits)), f2;
      }, br = (a10, b2, c2) => ["aes-128-ctr" === a10 ? { name: "AES-CTR", counter: b2.iv, length: 128 } : { name: "AES-CBC", iv: b2.iv }, b2.key, "string" == typeof c2 ? bh(c2) : c2], bs = async (a10, b2, c2, d2) => {
        let e2 = await bq(a10, b2, c2);
        return { encrypted: new Uint8Array(await a10.subtle.encrypt(...br(c2.algorithm, e2, d2))), key: e2 };
      }, bt = async (a10, b2, c2, d2) => {
        let e2, f2 = await bq(a10, b2, c2);
        return e2 = new Uint8Array(await a10.subtle.decrypt(...br(c2.algorithm, f2, d2))), new TextDecoder().decode(e2);
      }, bu = async (a10, b2, c2, d2) => {
        let e2 = await bq(a10, b2, { ...c2, hmac: true }), f2 = bh(d2);
        return { digest: bj(new Uint8Array(await a10.subtle.sign({ name: "HMAC" }, e2.key, f2))), salt: e2.salt };
      }, bv = (a10) => "string" == typeof a10 || a10 instanceof Uint8Array ? { encryption: a10, integrity: a10 } : "secret" in a10 ? { id: a10.id, encryption: a10.secret, integrity: a10.secret } : { id: a10.id, encryption: a10.encryption, integrity: a10.integrity }, bw = async (a10, b2, c2, d2) => {
        if (!c2) throw Error("Empty password");
        let e2 = bl(d2), f2 = Date.now() + (e2.localtimeOffsetMsec || 0), g2 = JSON.stringify(b2), { id: h2 = "", encryption: i2, integrity: j2 } = bv(c2);
        if (h2 && !/^\w+$/.test(h2)) throw Error("Invalid password id");
        let { encrypted: k2, key: l2 } = await bs(a10, i2, e2.encryption, g2), m2 = bj(new Uint8Array(k2)), n2 = bj(l2.iv), o2 = e2.ttl ? f2 + e2.ttl : "", p2 = `${bn}*${h2}*${l2.salt}*${n2}*${m2}*${o2}`, q2 = await bu(a10, j2, e2.integrity, p2);
        return `${p2}*${q2.salt}*${q2.digest}`;
      }, bx = async (a10, b2, c2, d2) => {
        if (!c2) throw Error("Empty password");
        let e2 = bl(d2), f2 = Date.now() + (e2.localtimeOffsetMsec || 0), g2 = b2.split("*");
        if (8 !== g2.length) throw Error("Incorrect number of sealed components");
        let h2 = g2[0], i2 = g2[1], j2 = g2[2], k2 = g2[3], l2 = g2[4], m2 = g2[5], n2 = g2[6], o2 = g2[7], p2 = `${h2}*${i2}*${j2}*${k2}*${l2}*${m2}`;
        if (bn !== h2) throw Error("Wrong mac prefix");
        if (m2) {
          if (!/^\d+$/.test(m2)) throw Error("Invalid expiration");
          if (Number.parseInt(m2, 10) <= f2 - 1e3 * e2.timestampSkewSec) throw Error("Expired seal");
        }
        let q2 = "";
        if (i2 = i2 || "default", "string" == typeof c2 || c2 instanceof Uint8Array) q2 = c2;
        else if (i2 in c2) q2 = c2[i2];
        else throw Error(`Cannot find password: ${i2}`);
        q2 = bv(q2);
        let r2 = e2.integrity;
        if (r2.salt = n2, !((a11, b3) => {
          let c3 = +(a11.length !== b3.length);
          c3 && (b3 = a11);
          for (let d3 = 0; d3 < a11.length; d3 += 1) c3 |= a11.charCodeAt(d3) ^ b3.charCodeAt(d3);
          return 0 === c3;
        })((await bu(a10, q2.integrity, r2, p2)).digest, o2)) throw Error("Bad hmac value");
        let s2 = bi(l2), t2 = e2.encryption;
        t2.salt = j2, t2.iv = bi(k2);
        let u2 = await bt(a10, q2.encryption, t2, s2);
        return u2 ? JSON.parse(u2) : null;
      };
      let by = globalThis.crypto, bz = by.subtle, bA = () => by.randomUUID(), bB = (a10) => by.getRandomValues(a10), bC = { randomUUID: bA, getRandomValues: bB, subtle: bz };
      var bD = { ttl: 1209600, cookieOptions: { httpOnly: true, secure: true, sameSite: "lax", path: "/" } };
      function bE(a10) {
        return "string" == typeof a10 ? { 1: a10 } : a10;
      }
      function bF(a10, b2) {
        if ("headers" in a10 && "function" == typeof a10.headers.append) return void a10.headers.append("set-cookie", b2);
        let c2 = a10.getHeader("set-cookie") ?? [];
        Array.isArray(c2) || (c2 = [c2.toString()]), a10.setHeader("set-cookie", [...c2, b2]);
      }
      function bG(a10) {
        let b2 = { ...bD, ...a10, cookieOptions: { ...bD.cookieOptions, ...a10.cookieOptions || {} } };
        if (a10.cookieOptions && "maxAge" in a10.cookieOptions) void 0 === a10.cookieOptions.maxAge && (b2.ttl = 0);
        else {
          var c2;
          b2.cookieOptions.maxAge = 0 === (c2 = b2.ttl) ? 2147483647 : c2 - 60;
        }
        return b2;
      }
      var bH = "iron-session: Bad usage: use getIronSession(req, res, options) or getIronSession(cookieStore, options).";
      async function bI(a10, b2, c2, d2) {
        if (!b2.cookieName) throw Error("iron-session: Bad usage. Missing cookie name.");
        if (!b2.password) throw Error("iron-session: Bad usage. Missing password.");
        let e2 = bE(b2.password);
        if (Object.values(e2).some((a11) => a11.length < 32)) throw Error("iron-session: Bad usage. Password must be at least 32 characters long.");
        let f2 = bG(b2), g2 = function(a11, b3) {
          let c3 = b3.get(a11), d3 = c3?.value;
          return "string" == typeof d3 ? d3 : "";
        }(f2.cookieName, a10), h2 = g2 ? await d2(g2, { password: e2, ttl: f2.ttl }) : {};
        return Object.defineProperties(h2, { updateConfig: { value: function(a11) {
          f2 = bG(a11);
        } }, save: { value: async function() {
          let b3 = await c2(h2, { password: e2, ttl: f2.ttl }), d3 = f2.cookieName.length + b3.length + JSON.stringify(f2.cookieOptions).length;
          if (d3 > 4096) throw Error(`iron-session: Cookie length is too big (${d3} bytes), browsers will refuse it. Try to remove some data.`);
          a10.set(f2.cookieName, b3, f2.cookieOptions);
        } }, destroy: { value: function() {
          Object.keys(h2).forEach((a11) => {
            delete h2[a11];
          });
          let b3 = { ...f2.cookieOptions, maxAge: 0 };
          a10.set(f2.cookieName, "", b3);
        } } }), h2;
      }
      var bJ = /* @__PURE__ */ function(a10, b2) {
        return c2;
        async function c2(c3, d2, e2) {
          var f2;
          if (!c3 || !d2) throw Error(bH);
          if (!e2) return bI(c3, d2, a10, b2);
          if (!e2) throw Error(bH);
          if (!e2.cookieName) throw Error("iron-session: Bad usage. Missing cookie name.");
          if (!e2.password) throw Error("iron-session: Bad usage. Missing password.");
          let g2 = bE(e2.password);
          if (Object.values(g2).some((a11) => a11.length < 32)) throw Error("iron-session: Bad usage. Password must be at least 32 characters long.");
          let h2 = bG(e2), i2 = (f2 = h2.cookieName, (0, be.q)(("headers" in c3 && "function" == typeof c3.headers.get ? c3.headers.get("cookie") : c3.headers.cookie) ?? "")[f2] ?? ""), j2 = i2 ? await b2(i2, { password: g2, ttl: h2.ttl }) : {};
          return Object.defineProperties(j2, { updateConfig: { value: function(a11) {
            h2 = bG(a11);
          } }, save: { value: async function() {
            if ("headersSent" in d2 && d2.headersSent) throw Error("iron-session: Cannot set session cookie: session.save() was called after headers were sent. Make sure to call it before any res.send() or res.end()");
            let b3 = await a10(j2, { password: g2, ttl: h2.ttl }), c4 = (0, be.l)(h2.cookieName, b3, h2.cookieOptions);
            if (c4.length > 4096) throw Error(`iron-session: Cookie length is too big (${c4.length} bytes), browsers will refuse it. Try to remove some data.`);
            bF(d2, c4);
          } }, destroy: { value: function() {
            Object.keys(j2).forEach((a11) => {
              delete j2[a11];
            }), bF(d2, (0, be.l)(h2.cookieName, "", { ...h2.cookieOptions, maxAge: 0 }));
          } } }), j2;
        }
      }(async function(a10, { password: b2, ttl: c2 = 1209600 }) {
        let d2 = bE(b2), f2 = Math.max(...Object.keys(d2).map(Number)), g2 = { id: f2.toString(), secret: d2[f2] }, h2 = await bw(e, a10, g2, { ...bk, ttl: 1e3 * c2 });
        return `${h2}~2`;
      }, async function(a10, { password: b2, ttl: c2 = 1209600 }) {
        let d2 = bE(b2), { sealWithoutVersion: f2, tokenVersion: g2 } = function(a11) {
          let [b3, c3] = a11.split("~");
          return { sealWithoutVersion: b3, tokenVersion: null == c3 ? null : parseInt(c3, 10) };
        }(a10);
        try {
          let a11 = await bx(e, f2, d2, { ...bk, ttl: 1e3 * c2 }) ?? {};
          if (2 === g2) return a11;
          return { ...a11.persistent };
        } catch (a11) {
          if (a11 instanceof Error && /^(Expired seal|Bad hmac value|Cannot find password|Incorrect number of sealed components)/.test(a11.message)) return {};
          throw a11;
        }
      });
      let bK = { current: null }, bL = "function" == typeof bd.cache ? bd.cache : (a10) => a10, bM = console.warn;
      function bN(a10) {
        return function(...b2) {
          bM(a10(...b2));
        };
      ...
      }), /* @__PURE__ */ new WeakMap(), bN(function(a10, b2) {
      ...
      let bO = { password: process.env.SESSION_SECRET || "complex_password_at_least_32_characters_long_for_security", cookieName: "acu_dashboard_session", cookieOptions: { secure: true, httpOnly: true, sameSite: "lax", path: "/", maxAge: 86400 } };
      async function bP(a10) {
        let { pathname: b2 } = a10.nextUrl;
        if (b2.startsWith("/login") || b2.startsWith("/api/auth") || b2.startsWith("/_next") || b2.startsWith("/static") || b2.includes(".")) return S.next();
        let c2 = S.next(), d2 = await bJ(a10, c2, bO);
        return (console.log("Middleware check:", { pathname: b2, isAuthenticated: d2.isAuthenticated, loginTime: d2.loginTime, cookies: a10.cookies.getAll().map((a11) => a11.name) }), d2.isAuthenticated) ? c2 : S.redirect(new URL("/login", a10.url));
      }
      let bQ = { matcher: ["/((?!_next/static|_next/image|favicon.ico).*)"] };
      Object.values({ NOT_FOUND: 404, FORBIDDEN: 403, UNAUTHORIZED: 401 });
      let bR = { ...f }, bS = bR.middleware || bR.default, bT = "/middleware";
      if ("function" != typeof bS) throw Object.defineProperty(Error(`The Middleware "${bT}" must export a \`middleware\` or a \`default\` function`), "__NEXT_ERROR_CODE", { value: "E120", enumerable: false, configurable: true });
      function bU(a10) {
        return bc({ ...a10, page: bT, handler: async (...a11) => {
          try {
            return await bS(...a11);
          } catch (e2) {
...
var ConfigHeaders = [];
var PrerenderManifest = { "version": 4, "routes": { "/": { "experimentalBypassFor": [{ "type": "header", "key": "Next-Action" }, { "type": "header", "key": "content-type", "value": "multipart/form-data;.*" }], "initialRevalidateSeconds": false, "srcRoute": "/", "dataRoute": "/index.rsc", "allowHeader": ["host", "x-matched-path", "x-prerender-revalidate", "x-prerender-revalidate-if-generated", "x-next-revalidated-tags", "x-next-revalidate-tag-token"] }, "/login": { "experimentalBypassFor": [{ "type": "header", "key": "Next-Action" }, { "type": "header", "key": "content-type", "value": "multipart/form-data;.*" }], "initialRevalidateSeconds": false, "srcRoute": "/login", "dataRoute": "/login.rsc", "allowHeader": ["host", "x-matched-path", "x-prerender-revalidate", "x-prerender-revalidate-if-generated", "x-next-revalidated-tags", "x-next-revalidate-tag-token"] } }, "dynamicRoutes": {}, "notFoundRoutes": [], "preview": { "previewModeId": "4de4c1d932595110cca2f00f71a091ba", "previewModeSigningKey": "523172f66e459e0b6f2c449ece82c3392faed71c6d8bc3b48d9fa079509c4b91", "previewModeEncryptionKey": "5694d488b9bf34d0db009d3c13c6a74fc8ce5c9efb1edce6628aa2ed84431961" } };
var MiddlewareManifest = { "version": 3, "middleware": { "/": { "files": ["server/edge-runtime-webpack.js", "server/middleware.js"], "name": "middleware", "page": "/", "matchers": [{ "regexp": "^(?:\\/(_next\\/data\\/[^/]{1,}))?(?:\\/((?!_next\\/static|_next\\/image|favicon.ico).*))(\\.json)?[\\/#\\?]?$", "originalSource": "/((?!_next/static|_next/image|favicon.ico).*)" }], "wasm": [], "assets": [], "env": { "__NEXT_BUILD_ID": "TmSA8UNLlM1RD_YYXM_qs", "NEXT_SERVER_ACTIONS_ENCRYPTION_KEY": "W8oYqlcg6mc5tgH6/CFZZjJ24XNeL6Fi5Wqru4OUkiY=", "__NEXT_PREVIEW_MODE_ID": "4de4c1d932595110cca2f00f71a091ba", "__NEXT_PREVIEW_MODE_SIGNING_KEY": "523172f66e459e0b6f2c449ece82c3392faed71c6d8bc3b48d9fa079509c4b91", "__NEXT_PREVIEW_MODE_ENCRYPTION_KEY": "5694d488b9bf34d0db009d3c13c6a74fc8ce5c9efb1edce6628aa2ed84431961" } } }, "functions": {}, "sortedMiddleware": ["/"] };
var AppPathRoutesManifest = { "/api/auth/check/route": "/api/auth/check", "/api/auth/login/route": "/api/auth/login", "/api/auth/logout/route": "/api/auth/logout", "/_not-found/page": "/_not-found", "/api/crosstab/route": "/api/crosstab", "/api/dashboard/route": "/api/dashboard", "/login/page": "/login", "/page": "/" };
