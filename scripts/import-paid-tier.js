...
async function main() {
  console.log('📦 Importing all transactions to remote D1 (paid tier)...\n');
  ...
  for (const filename of transFiles) {
    ...
    const ROWS_PER_INSERT = 100; // More rows per insert with paid tier
    const INSERTS_PER_BATCH = 25; // 25 inserts per batch = 2500 rows
    const DELAY_MS = 500; // Shorter delay with paid tier
    ...
    for (let i = 0; i < sqlLines.length; i += INSERTS_PER_BATCH) {
      ...
      try {
        ...
        process.stdout.write(`\r  Progress: ${Math.min(i + INSERTS_PER_BATCH, sqlLines.length)}/${sqlLines.length} (${successCount} success, ${errorCount} errors)  `);
        await sleep(DELAY_MS * 2); // Longer delay on error
      }
    }
